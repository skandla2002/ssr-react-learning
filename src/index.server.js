...
const stateFile = path.resolve('./build/loadable-state.json');
...
const serverRender = (req, res, next) => {
  ...
  store.dispatch(END); // redux-saga의 END 액션을 발생시키면 액션을 모니터링 하는 사기들이 모두 종료됩니다.
  try{
    await sagaPromise; // 기존에 진행중이던 사가들이 모두 끝날때 까지 기다립니다.
  ...
  const tags = {
    scripts: stateScript + extractor.getScriptTags(), // 스크립트 앞부분에 리덕스 상태넣기
